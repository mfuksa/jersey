...
public class OAuthClientServerTest extends JerseyTest {
    ...
    public void testAuthorizationFlow() {
        final OAuthClientFeature clientFeature = getClientFeature();

        Client client = ClientBuilder.newBuilder().register(clientFeature).build();
    ...
    public void testAuthorizationFlowWithFeatureRegisteredToTarget() {
        ...
        final OAuthClientFeature oAuthClientFeature = getClientFeature();
        Client client = ClientBuilder.newBuilder().build();

        Response response = client.target(getBaseUri()).path("resource").register(oAuthClientFeature)
                .request().get();
        Assert.assertEquals(200, response.getStatus());
        Assert.assertEquals("homer", response.readEntity(String.class));

        response = client.target(getBaseUri()).path("resource").path("admin").register(oAuthClientFeature)
                .request().get();
        Assert.assertEquals(200, response.getStatus());
        Assert.assertEquals(false, response.readEntity(boolean.class));
    }

    private OAuthClientFeature getClientFeature() {
        ...
        final OAuthClientFeature feature = OAuthClientFeature.builder(CONSUMER_KEY, SECRET_CONSUMER_KEY)
                .buildAuthorizationFlowMode(tempCredUri, accessTokenUri, userAuthorizationUri, authorizationHandler,
                        "http://consumer/callback/homer");
        return feature;
    }
    ...
    public void testRequestSigning() {
        final OAuthClientFeature oAuthClientFeature = OAuthClientFeature.builder(CONSUMER_KEY, SECRET_CONSUMER_KEY)
                .buildSignatureModeFeature(PROMETHEUS_TOKEN, PROMETHEUS_SECRET);
        final Client client = ClientBuilder.newBuilder()
                .register(oAuthClientFeature).build();
