...
class OAuth2ClientFilter implements ClientRequestFilter {
    ...
    public void filter(ClientRequestContext request) throws IOException {
        ...
        final String propertyToken = (String) request.getProperty(OAuth2ClientFilterFeature.OAUTH2_PROPERTY_ACCESS_TOKEN);
        if (propertyToken != null) {
            token = propertyToken;
        }
        request.removeProperty(OAuth2ClientFilterFeature.OAUTH2_PROPERTY_ACCESS_TOKEN);
        if (token == null) {
            return;
//            throw new ProcessingException("Missing Access Token in the OAuth2 filter.");
        }
