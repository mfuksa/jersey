...
import org.glassfish.jersey.server.oauth.internal.AccessTokenResource;
import org.glassfish.jersey.server.oauth.internal.RequestTokenResource;

/**
 * The feature enables support for OAuth 1.0a on the server.
 * <p/>
 * The OAuth 1 server support requires implementation of {@link OAuthProvider};
 ...
 * can be overwritten by configuration properties {@link OAuthProperties#ENABLE_TOKEN_RESOURCES},
 * {@link OAuthProperties#REQUEST_TOKEN_URI} and {@link OAuthProperties#ACCESS_TOKEN_URI}.
 *
 * @author Miroslav Fuksa (miroslav.fuksa at oracle.com)
 */
public class OAuthServerFeature implements Feature {

    private final OAuthProvider oAuthProvider;
    private final boolean enableTokenResources;
    private final String requestTokenUri;
    private final String accessTokenUri;

    /**
     * Create a new feature configured with {@link OAuthProvider OAuth provider} and request and access token
     ...
     *
     * @param oAuthProvider Instance of the {@code OAuthProvider} that will handle authorization. If the value is
     ...
     */
    public OAuthServerFeature(OAuthProvider oAuthProvider,
                              String requestTokenUri,
                              String accessTokenUri) {
        this.oAuthProvider = oAuthProvider;
        this.enableTokenResources = true;
        this.requestTokenUri = requestTokenUri;
        this.accessTokenUri = accessTokenUri;
    }

    /**
     * Create a new feature configured with {@link OAuthProvider OAuth provider}. The feature will not
     ...
     *
     * @param oAuthProvider Instance of the {@code OAuthProvider} that will handle authorization.
     */
    public OAuthServerFeature(OAuthProvider oAuthProvider) {
        this(oAuthProvider, null, null);
    }

    /**
     * Create a new feature. The feature will not register any {@link OAuthProvider OAuth provider}
     * and it will not expose Request and Access Token Resources. {@code OAuthProvider} must be registered
    ...
    public boolean configure(FeatureContext context) {
        if (oAuthProvider != null) {
            context.register(oAuthProvider);
        }

        context.register(OAuthServerFilter.class);

        if (!context.getConfiguration().isRegistered(OAuthSignatureFeature.class)) {
        ...
        final Boolean propertyResourceEnabled = PropertiesHelper.getValue(properties,
                OAuthProperties.ENABLE_TOKEN_RESOURCES, null, Boolean.class);

        boolean registerResources = propertyResourceEnabled != null ?
                propertyResourceEnabled : enableTokenResources;

        if (registerResources) {
            String requestUri = PropertiesHelper.getValue(properties, OAuthProperties.REQUEST_TOKEN_URI,
                    null, String.class);
            if (requestUri == null) {
                requestUri = requestTokenUri == null ? "requestToken" : requestTokenUri;
            }

            String accessUri = PropertiesHelper.getValue(properties, OAuthProperties.ACCESS_TOKEN_URI,
                    null, String.class);
            if (accessUri == null) {
