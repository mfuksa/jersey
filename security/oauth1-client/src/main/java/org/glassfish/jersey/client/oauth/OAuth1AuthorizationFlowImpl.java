...
class OAuth1AuthorizationFlowImpl implements OAuth1AuthorizationFlow {
    ...
    private final Value<OAuth1ClientFilterFeature> oAuth1ClientFilterFeature = new Value<OAuth1ClientFilterFeature>() {
        @Override
        public OAuth1ClientFilterFeature get() {
            return OAuth1ClientSupport.builder(consumerCredentials).buildFilterFeature()
    ...
    private OAuth1AuthorizationFlowImpl(ConsumerCredentials consumerCredentials, String requestTokenUri,
                                        ...
                                        Client client) {
        this.parameters = new OAuthParameters().consumerKey(consumerCredentials.getConsumerKey());
        this.secrets = new OAuthSecrets().consumerSecret(consumerCredentials.getConsumerSecret());
        this.consumerCredentials = consumerCredentials;
        this.requestTokenUri = requestTokenUri;
        this.accessTokenUri = accessTokenUri;
        this.authorizationUri = authorizationUri;
        ...
        // TODO: register to WebTarget instead of this client.
        if (!this.client.getConfiguration().isRegistered(OAuth1ClientFilterFeature.class)) {
            final OAuth1ClientFilterFeature filterFeature = OAuth1ClientSupport.builder(consumerCredentials)
                    .buildFilterFeature().build();
    ...
    private Invocation.Builder addPropeties(Invocation.Builder invocationBuilder) {
        return invocationBuilder
                .property(OAuth1ClientFilterFeature.OAUTH_PROPERTY_OAUTH_PARAMETERS, parameters)
                .property(OAuth1ClientFilterFeature.OAUTH_PROPERTY_OAUTH_SECRETS, secrets);
    }
    ...
    public String start() {
        ...
        parameters.token(formParams.getFirst(OAuthParameters.TOKEN));
        secrets.tokenSecret(formParams.getFirst(OAuthParameters.TOKEN_SECRET));

        return UriBuilder.fromUri(authorizationUri).queryParam(OAuthParameters.TOKEN, parameters.getToken())
                .build().toString();
    }
    ...
    public Client getAuthorizedClient() {
        return ClientBuilder.newClient().register(getOAuth1FilterFeature());
    }

    @Override
    public OAuth1ClientFilterFeature getOAuth1FilterFeature() {
        if (this.accessToken == null) {
            throw new IllegalStateException("Authorization is not finished and access token was not received. Call finish() to finish the authorization.");
        }
